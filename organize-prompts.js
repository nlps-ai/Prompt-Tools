...
const organizedPrompts = [
  ...
  {
    name: "小红书爆款文案生成器",
    content: "你是小红书平台的内容创作专家，深谙平台用户喜好和算法规则。请帮我创作一篇能够获得高互动的小红书文案。\n\n创作要点：\n1. 开头要有吸引力，制造悬念或共鸣\n2. 内容要有价值，提供实用信息或情感价值\n3. 使用合适的emoji和分段\n4. 包含相关话题标签\n5. 结尾引导互动（点赞、收藏、评论）\n\n请告诉我内容主题和目标受众。",
    tags: ["小红书", "文案", "社交媒体", "营销"],
    source: "内部整理",
    notes: "专门针对小红书平台优化"
  },
...
function deduplicatePrompts(prompts) {
  ...
  for (const prompt of prompts) {
    // 基于名称和内容的前100个字符创建唯一标识
    const identifier = `${prompt.name}_${prompt.content.substring(0, 100)}`;
...
function categorizePrompts(prompts) {
  ...
  prompts.forEach(prompt => {
    prompt.tags.forEach(tag => {
      ...
      categories[tag].push(prompt.name);
    });
  });
...
// 生成统计报告
function generateReport(prompts) {
  ...
  Object.entries(categories).forEach(([category, items]) => {
    report.categoryBreakdown[category] = items.length;
  });
...
function main() {
  ...
  // 去重处理
  const uniquePrompts = deduplicatePrompts(organizedPrompts);
  console.log(`去重后提示词数量: ${uniquePrompts.length}`);
  
  // 生成报告
  const report = generateReport(uniquePrompts);
  console.log('统计报告:', JSON.stringify(report, null, 2));
  
  // 导出数据
  const exportData = exportToJSON(uniquePrompts);
  
  // 保存到文件
  const outputPath = path.join(__dirname, 'initial-prompts.json');
  fs.writeFileSync(outputPath, JSON.stringify(exportData, null, 2), 'utf8');
  
  console.log(`初始化提示词已保存到: ${outputPath}`);
  console.log(`总计 ${exportData.prompts.length} 个高质量提示词`);
  
  // 生成分类统计文件
  const statsPath = path.join(__dirname, 'prompts-stats.json');
  fs.writeFileSync(statsPath, JSON.stringify(report, null, 2), 'utf8');
