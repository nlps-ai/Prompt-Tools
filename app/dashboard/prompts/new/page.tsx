          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-4">
                ...
                {/* Common Tags */}
                <div>
                  <p className="text-sm text-gray-500 mb-2">常用标签：</p>
                  <div className="flex flex-wrap gap-2">
                    {['写作', '翻译', '编程', '分析', '创意', '商务', '教育', '娱乐'].map((commonTag) => (
                      <Button
                        key={commonTag}
                        type="button"
                        variant="outline"
                        size="sm"
                        onClick={() => {
                          if (!tags.includes(commonTag)) {
                            const newTags = [...tags, commonTag]
                            setTags(newTags)
                            form.setValue('tags', newTags)
                          }
                        }}
                        disabled={tags.includes(commonTag) || isLoading}
                        className="text-xs"
                      >
                        {commonTag}
                      </Button>
